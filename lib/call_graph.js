...
function x(node,call_graph,tot_time)
{
	node.total_time_pc = (tot_time-node.total_time_ms)/tot_time*100;
	node.self_time_pc = (tot_time-node.self_time_ms)/tot_time*100;
	
	for(var key in node.children)
	{
	    if (typeof callgraph[key] === 'object')
		{
			x(callgraph[key],call_graph,tot_time);
		}
	}
}
