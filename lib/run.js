...
function PerfData ()
{
  ...
  this.process_data_async = function ()
  {
    ...
    for (var i = 0; i < self.perf_data_raw.length; ++i)
    { 
      ...
      self.perf_data[key].ncalls += o.delta;
    }
  };


  this.process_data = function (caller, callee, time, delta)
  {
    var o = { 
              caller : caller,
              callee : callee,
...
{
  // if no delta was received then it defaults to 1
  if (typeof delta !== 'number')
  {
    delta = 1;
  }

  pd.process_data (caller, callee, time, delta);
}
...
function __$__m_exit__$__ (name, line, index)
{
  ...
  // Determine caller
  var caller = (call_stack.length) ? call_stack[call_stack.length - 1].f_info :
                                     { name : GLOBAL_FUNCTION_STR, line : '0', index : '0'};
